...
public class ItunesDAO {
	...
	public List<Track>getVertici(Genre genere, Map<Integer,Track>idMap){
		String sql ="SELECT TrackId FROM track WHERE GenreId =?";
	...
	public List<Adiacenza> getArchi(Genre genere, Map<Integer,Track>idMap){
		String sql ="SELECT t1.TrackId AS t1, t2.TrackId AS t2, abs(t1.milliseconds - t2.milliseconds) AS delta FROM track t1, track t2 WHERE t1.TrackId> t2.TrackId AND t1.MediaTypeId = t2.MediaTypeId AND t1.GenreId =? AND t1.GenreId = t2.GenreId";
		...
		try {
			...
			while(res.next()) {
				result.add(new Adiacenza(idMap.get(res.getInt("t1")),idMap.get(res.getInt("t2")),res.getDouble("delta")));
			}
			conn.close();
